...
const EVENT_MAP = {
  ...
  '#paper-height': {
    on: 'change',
    action: (e) => {
      document.querySelector('.page-a').style.height = e.target.value + 'px';
    }
  },
  '#heading-color': {
    on: 'change',
    action: (e) => {
      document.querySelector('.page-a').style.setProperty('--heading-ink-color', e.target.value);
    }
  },
  '#font-file': {
    on: 'change',
  ...
  '#text-input': {
    ...
    action: (e) => {
      ...
      window.localStorage.setItem('saved-text', text);
      
      const html = text.replace(/</g, '&lt;').replace(/>/g, '&gt;').split('\n').map(line => {
        if (line.startsWith('# ')) {
          return `<span style="color: var(--heading-ink-color, inherit);">${line.substring(2)}</span>`;
        }
        return line;
      }).join('<br>');
...
// Initialize textarea with default paper text or saved text
const textInput = document.querySelector('#text-input');
const note = document.querySelector('#note');
if (textInput && note) {
  const saved = window.localStorage.getItem('saved-text');
  if (saved) {
    textInput.value = saved;
  } else {
    textInput.value = note.innerText;
  }
  // Render html content cleanly on initial load
  EVENT_MAP['#text-input'].action({ target: textInput });
}
